...
class Network(object):
    def __init__(self, dataset=None, trainable=True):  # data_shape = (height, width)
        ...
        self.__input_layer = tf.placeholder(tf.float32, shape=[None, self.__data_shape[0], self.__data_shape[1],
                                                               self.__data_shape[2]], name='input')
    ...
    def weight_variable(self, shape, variable_name, scope, collection, reuse, trainable):
        ...
        with tf.variable_scope('weight', reuse=reuse):
            var = tf.get_variable(scope + '/' + variable_name, shape=shape, trainable=trainable, collections=collection,
    ...
    def bias_variable(self, shape, variable_name, scope, collection, reuse, trainable):
        ...
        with tf.variable_scope('bias', reuse=reuse):
            var = tf.get_variable(scope + '/' + variable_name, trainable=trainable, shape=shape, collections=collection,
                                  regularizer=regularizer)
    ...
    def conv2d_tran(self, input_data, k_w, k_d, s_w, s_h, name, output_shape=None,
                    scope=None, collection=None, relu=True, padding='SAME', appendList=None, reuse=False,
                    trainable=True):
    ...
    @decorated_layer
    def fc(self, input_data, output_dim, name, collection=None, scope=None, relu=True, appendList=None, reuse=False,
           trainable=True):
    ...
    def weight_sum(self, input_data, name, collection, stddev=0.1):
        ...
        weight_variables = [tf.multiply(weight_variable(shape, collection, stddev=stddev), input_data[i]) for i in
                            len(input_data)]
        return tf.reduce_sum(weight_variables, axis=1)

    @decorated_layer
    def batch_normalization(self, input_data, name, scope=None, relu=True, decay=0.9, epsilon=1e-5,
                            updates_collections=tf.GraphKeys.UPDATE_OPS, trainable=False, appendList=None, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            if reuse == True:
                assert not scope is None
            temp_layer = tf.contrib.layers.batch_norm(input_data, decay=decay, scale=True,
                                                      center=True, variables_collections=scope, epsilon=epsilon,
